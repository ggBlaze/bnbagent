...
[![Tests](https://img.shields.io/badge/tests-775%2F787%20passing-brightgreen)](tests/)
...
> **Reproducibility / judges:** the backtest table in [`docs/demo-script.md`](docs/demo-script.md) is generated by `python -m scripts.run_both_regimes` (3 regimes × 2 tape intervals, `equity=1000`). `bash bnbagent --replay` runs a single 7-day synthetic regime with the default `equity=100` — useful as a quick smoke test, but its numbers won't match the table (different position sizing → different trade count → different Sharpe). To audit the math, open the JSON in `data/reports/` and check the `samples_per_year` field — that's the actual annualization denominator (v2.1.5: was hardcoded to 525,600, now computed from the tape duration / equity-curve length).
...
│   └── replay.py
│
├── tests/                         ← 775/787 passing (enforced by CI)
│   ├── unit/                      ← ~13 files
...
```bash
pytest -q                          # 775/787 passing (~3:40 cold, ~12s unit-only)
