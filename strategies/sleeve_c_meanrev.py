...
class SleeveCMeanRev:
    ...
    async def _open_mean_rev(self, sym: str, ref_price: float, sigma: float, equity: Decimal, sleeve_cfg: dict):
        ...
        # as a uint24 in the calldata; -1 throws ValueOutOfBounds.
        if pool_fee is None or pool_fee < 0:
            log.info(f"Sleeve C skip {sym}: no working pool for USDC->{sym}")
            return
        # v2.2.4 (decimals bugfix): USDC has 18 decimals on BSC
        # mainnet (was hardcoded as 6). Same as sleeve_a/b.
        from core.utils import token_decimals
        usdc_decimals = token_decimals("USDC", self.cfg)
        amount_in = int(size * Decimal(10 ** usdc_decimals))
        ...
        try:
            ...
            log.warning(f"Sleeve C resync_nonce failed: {e}")
        ...
        try:
            ...
            log.info(f"Sleeve C {sym}: broadcast aborted — {e}")
            return
